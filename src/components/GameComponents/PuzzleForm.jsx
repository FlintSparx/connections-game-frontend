...
function PuzzleForm({ 
  ...
  const [categories, setCategories] = useState(() => {
    ...
    const defaultCategories = Array(4).fill().map(() => ({...emptyCategory}));
  ...
  return (
    <form onSubmit={handleSubmit} style={{ 
      ...
      <div style={{ marginBottom: 16 }}>
        <label htmlFor="gameName" style={{ display: 'block', marginBottom: 4, fontWeight: 600 }}>
          Game Board Name
        </label>
        <input
          type="text"
        ...
        {categories.map((cat, catIdx) => (
          <div key={catIdx}>
            <div style={{
              ...
              <span style={{ marginRight: 8 }}>{DIFFICULTY_LABELS[catIdx]}</span>
              <input
                type="text"
